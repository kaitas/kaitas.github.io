...
プログラミングをやめて，コメントやWikiを書くつもりで，コードの設計や，開発環境のセットアップなどを書き始めるのもエンジンがかかる．

\footnote{実際には「開発環境などは付録に書け」と指導教員の指導を受けるだろう．だとしても，冬休み期間中に1ミリも執筆が進まないよりは，先生の血圧計は高まらないことが経験的に報告されている．}

## \subsection{写真を撮れ}

まずは動いているものがあれば，写真を撮ろう．スクリーンショットを撮ろう．ソースコードの大事な場所や体験者の様子など，
わかりやすい図（`\textit{key figure}`）を配置して，キャプションに，言いたかったことをしっかりメモしておこう．
...
例えば，ある実験のパラメータを `$\left\{X,Y\right\}$` とし，被験者が `5名` `$\left\{A,B,C,D,E\right\}$` といたとしよう．
...
- たまには印刷してみる．紙のほうがはるかにはかどる（A5サイズなどでよい）．
- 先生が修正指摘した箇所は二度と修正候補にならないように確実に修正する
- 日付等のバージョン番号で管理し，古いバージョンのTeXは削除や変更ではなく保存しておく
- 本気で最終版ならバインダーに入れて
- 先生がブレることもある
...
`\usepackage{makeidx}` の下，`\makeindex` の上に以下を書くと良いそうです．
...
- 20200616 - サーバ運営方針変更にあわせて[本家のWodpress](http://aki.shirai.as/tex-sample/)よりGithub PagesのMarkdownに移植。TeX混じりなので、スタイルが一部変だがまあ楽しんでほしい。
